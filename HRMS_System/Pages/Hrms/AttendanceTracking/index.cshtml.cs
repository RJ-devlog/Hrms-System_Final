...
namespace HRMS_System.Pages.Dashboard.AttendanceTracking
{
    ...
    public class AttendanceTrackingPageModel : PageModel
    {
        ...
        public async Task OnGetAsync(string? summaryMonth)
        {
            ...
            var monthStart = SummaryMonth.Date;
            var monthEndExclusive = monthStart.AddMonths(1);

            //LOAD Date Tracking (default = current month)
            DateRangeAttendance = await _context.AttendanceTrackings
                .AsNoTracking()
                .Include(a => a.User) // REQUIRED because your Razor uses a.User.*
                .Where(a => a.AttendanceDate >= monthStart && a.AttendanceDate < monthEndExclusive)
                .OrderByDescending(a => a.AttendanceDate)
                .ThenByDescending(a => a.TimeIn)
                .ToListAsync();

            //LOAD Today Attendance (optional if you want a dedicated list)
            TodayAttendance = await _context.AttendanceTrackings
                .AsNoTracking()
                .Where(a => a.AttendanceDate == today)
                .ToListAsync();

            TotalWorkDays = Enumerable.Range(1, DateTime.DaysInMonth(monthStart.Year, monthStart.Month))
            ...
            AttendanceSummary = await _context.AttendanceTrackings
                ...
                .Select(g => new AttendanceSummaryRow
                {
                    ...
                    Absent = TotalWorkDays
                              - g.Select(x => x.AttendanceDate.Date).Distinct().Count()
                })
